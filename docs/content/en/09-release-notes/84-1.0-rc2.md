...
Highlights of this release are:
 ...
 - **Improved pre-activation checks**. One of the biggest source of bug reports in previous releases was when VersionPress was installed into an environment that technically worked but was not really supported. We are continually implementing more checks, for example, in RC2 we now check that the site is not Multisite, that the number of db entities is reasonable, that the site is run on a supported web server, etc. 
