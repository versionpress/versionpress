...
## New in 2.1
 ...
 - `vp clone` command validates the `--name` parameter to be applicable as a directory name, domain name, DB prefix etc. (#530)
 - IIS / Windows Server issues explored and documented (#522)
 - Other small fixes:
...
VersionPress 2.1 is *not* upgradable from 2.0.1 and previous versions, full re-activation is required. The recommended procedure is:
